  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8000/smartthings/integrations",
        {
          userId,
          integrationName,
          signal,
  ...
  return (
    <div className="flex flex-col space-y-4 overflow-scroll h-full">
      <form onSubmit={handleSubmit} className="flex flex-col space-y-4">
        ...
        <div className="flex flex-col space-y-2">
          <label htmlFor="integrationName" className="font-bold text-blue-500">
            Integration Name:
          </label>
          <input
            type="text"
          ...
          {devices.map((device, index) => (
            <div className="flex flex-col space-y-2" key={index}>
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange(index, "deviceId", e.target.value)
                }
                className="border border-gray-300 rounded-md p-2"
              />
              <label
        ...
        <button
          type="button"
          onClick={() => setDevices([...devices, { deviceId: "", state: "" }])}
